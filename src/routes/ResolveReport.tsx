  ...
  return !contextValue.sessionToken ? (
    ...
    >
      ...
      <TextField
        ...
        fullWidth={true}
        label={"Причина жалобы:"}
        value={`Администратор: ${reportUsers.admin}\nРешение: ${report.status === "B" ? "Забанен" : "Недостаточно оснований"}\nДата решения: ${dayjs(report.updatedAt).isValid() ? dayjs(report.updatedAt).format("YYYY-MM-DD HH:mm:ss") : ""}`}
        disabled={true}
      />
      <Button
        sx={{
        ...
        color={report.status === "B" ? "success" : "error"}
        onClick={() => {
          if (report.status === "B") {
      ...
      >
        {report.status === "B"
          ? `Изменить решение на: 'Недостаточно доказательств'`
          : `Изменить решение на: 'Забанить'`}
      </Button>
      <Dialog
      ...
      >
        ...
        <DialogContent id={"alert-dialog-description"}>
          {`Изменить решение на 'Недостаточно оснований для наказания?'`}
        </DialogContent>
        <DialogActions>
          <Button
            variant={"text"}
          ...
          <Button
            ...
            onClick={() => {
              ...
              axios
                ...
                .then((value) => {
                  enqueueSnackbar("Решение успешно изменено", {
                    variant: "success",
                ...
                .catch((reason) => {
                  enqueueSnackbar(
                    `Ошибка изменения решения${typeof reason.response?.data === "string" && (reason.response.data as string).length ? `: ${reason.response.data as string}` : ""}`,
                    {
                      variant: "error",
                      autoHideDuration: 3000,
                    },
